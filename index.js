...
function nextNewYear() {
  ...
  const newYear = new Date(yearNow + 1, 0, 1);
  // Визначаємо кількість мілісекунд до наступного Нового року в змінну diff.
  const diff = {
    Дні: ((newYear - date) / (1000 * 60 * 60 * 24)).toFixed(),
    Години: (((newYear - date) / (1000 * 60 * 60)) % 24).toFixed(),
    Хвилини: (((newYear - date) / (1000 * 60)) % 60).toFixed(),
    Секунди: (((newYear - date) / 1000) % 60).toFixed(),
    Мілісекунди: (newYear - date) % 1000,
  };
  ...
  return { "До Нового Года осталось:": diff };
}
...
function addDays(date, days) {
  ...
  ("Помилка: кількість днів має бути числом");
...
function getDayOfWeek(date) {
  ...
  return { inputDate: date.toISOString(), dayOfWeek };
}
...
function getDaysInMonth(date) {
  ...
  const daysInMonth = new Date(year, newDate.getMonth(), newDate.getDate() - 1);
...
function getFormattedTime(date) {
  ...
  // Збереження початкової дати для виведення  в форматі ISO.
  // Отримання годин.
  let formattedTime = date.toTimeString();
  // Отримання хвилин.
  // Отримання секунд.
...
function getAge(birthDate) {
  ...
  if (date.getMonth() < birthDate.getMonth()) age - 1;
...
function compareDates(date1, date2) {
  ...
  // "Помилка: вхідні значення мають бути об'єктами Date"
